...
export class CalendarComponent implements OnInit {
  ...
  upcomingEvents: any[] = [];
  todayEvents : any[] = [];
// Array to store the fetched calendar items


  // public nextDayEvents$: BehaviorSubject<UpcomingEvent[]> = new BehaviorSubject<UpcomingEvent[]>([]);
...
private searchItemsOnFriday(): void {
  const itemsOnFriday = this.calendaritems.filter((item) => {
    const itemDate = new Date(item.date);
    return itemDate.getDay() === 5; // 5 corresponds to Friday (Sunday is 0, Monday is 1, ..., Saturday is 6)
  });


  console.log('Items on Friday:', itemsOnFriday);
}
